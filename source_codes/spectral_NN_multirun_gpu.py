def spectral_NN_multirun_gpu(method,M,L,depth,width,q,replicates=range(25)):
    import os, sys, time
    import numpy as np
    import torch
    ...
    import Important_functions_gpu as Ifn
    ...
    for repl in replicates:
        ...
        loss = Ifn.loss_spectralNN(N, setup.wt_fn, grid_size=setup.loss_grid, q=q, device=device)
        ...
        spect_dens_est = Ifn.spectral_density_evaluation(model, q=q, wt_fn=setup.wt_fn, device=device)
        ...
        start_time = time.time()
        test_err,num,den,tr_cospect,tr_quadspect,err_cospect,err_quadspect = Ifn.spectral_error_computation(spect_dens_est,theta_file,loc_file,spect_file,device=device)
