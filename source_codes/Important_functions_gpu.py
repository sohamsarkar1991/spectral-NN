...
class loss_spectralNN:
    ...
    def inner_part(self, x, x_tilde):
        ...
        A = torch.zeros([2*self.q+1,2*self.q+1],dtype=torch.float32,requires_grad=False).to(self.device)
    ...
    def loss_fn(self, x, x_tilde):
        ...
        for theta in self.thetas:
            l_ = torch.einsum("i,ij,j ->", self.w,torch.cos(theta*self.C_diff)*A,self.w)
            if l_ > 0.:
                l += torch.sqrt(l_)/(2*np.pi)
        return l
...
class spectral_density_evaluation:
    ...
    def __init__(self, model, q, wt_fn, device="cpu"):
        ...
        hs = np.arange(start=-self.q,stop=self.q+0.5,step=1.,dtype="float32")
        self.w = torch.from_numpy(wt_fn(hs/self.q)).to(device)
        self.hs = torch.from_numpy(hs).to(device)
        
        self.lam = torch.empty([self.M,self.M,2*self.L+1,2*self.L+1,2*self.q+1],dtype=torch.float32,requires_grad=False).to(device)
        with torch.no_grad():
...
##### Error computation #####
def spectral_error_computation(spect_est,theta_file="True_thetas.dat",loc_file="True_locations.dat",spect_file="True_spectrum.dat",device="cpu"):
    ...
    tr_thetas = torch.from_numpy(tr_thetas).to(device)
    tr_loc = torch.from_numpy(tr_loc).to(device)
    spect_tr = torch.from_numpy(spect_tr).to(device)
...
def spect_norm(spect_dens,theta,u_,v_,D_):
    #u_ = np.random.uniform(low=0.,high=1.,size=(D_,d))
    #v_ = np.random.uniform(low=0.,high=1.,size=(D_,d))
    #u_ = np.asarray(u_, dtype="float32")
    #v_ = np.asarray(v_, dtype="float32")
    #u_ = torch.from_numpy(u_)
    #v_ = torch.from_numpy(v_)
    spect = spect_dens.evaluate(theta,u_,v_)
    co_nrm2 = torch.sum(spect[:,0]**2)/D_
    quad_nrm2 = torch.sum(spect[:,1]**2)/D_
    nrm = torch.sqrt(co_nrm2 + quad_nrm2)
    co_nrm, quad_nrm = torch.sqrt(co_nrm2), torch.sqrt(quad_nrm2)
    return [co_nrm.item(), quad_nrm.item(), nrm.item()]
